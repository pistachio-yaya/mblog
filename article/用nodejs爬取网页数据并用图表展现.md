...
## 前言
网络上充斥着大量的信息，数据分析是现在比较火的方向，在资金不足的情况下，很多公司都会采取众筹的方式，筹集资金来开发产品，猫猫 的这篇文章算不上是真正的爬虫工具，但是确能完美的实现拿到数据并展现的功能，如果你感兴趣，也可以资金试试看。

## 说明
本文以爬取京东众筹智能门锁页为例，展现爬取的实现方法。尝试的话也可以以其它网页为目标对象，都是可以实现的。其中的主要思想就是用node发送http请求去请求目标地址，这个url、method、port、host等都可以在网页的请求头中看到，利用这些信息我们就可以轻松的假装自己是京东众筹的前端给后端发请求拿数据了，哈哈哈哈～～～～喵(淡定)。
...
```
以上久完成了发送http请求的部分，那么接下来猫猫要考虑的是怎么把这个数据传到页面上。作为一个前端很容易想到，前端页面是通过发送http请求来获取数据的，所以我们现在还需要一个服务器资源供前端请求数据用，那么node的http模块的另一个作用就体现出来了，so，看下面就知道了。
